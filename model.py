...
class Model:
    ...
    def __init__(self, model_name, model_type, params, model_id, dataset_id, X, y):
    ...
    def linear_discriminant_analysis(self):
        return LinearDiscriminantAnalysis(solver=self.params['solver'], shrinkage=self.params['shrinkage'],
                                          n_components=self.params['n_components'], tol=self.params['tol'])

    def support_vector_classification(self):
        return SVC(C=self.params['C'], kernel=self.params['kernel'], degree=self.params['degree'],
                   gamma=self.params['gamma'], coef0=self.params['coef0'], shrinking=self.params['shrinking'],
                   tol=self.params['tol'], max_iter=self.params['max_iter'])

    def decision_tree_classification(self):
        return DecisionTreeClassifier(criterion=self.params['criterion'], splitter=self.params['splitter'],
    ...
    def k_nearest_neighbors(self):
        return KNeighborsClassifier(n_neighbors=self.params['n_neighbors'], weights=self.params['weights'],
                                    algorithm=self.params['algorithm'], leaf_size=self.params['leaf_size'],
                                    p=self.params['p'])

    def logistic_regression(self):
        return LogisticRegression(penalty=self.params['penalty'], dual=self.params['dual'], tol=self.params['tol'],
                                  C=self.params['C'], fit_intercept=self.params['fit_intercept'],
                                  intercept_scaling=self.params['intercept_scaling'], l1_ratio=self.params['l1_ratio'],
