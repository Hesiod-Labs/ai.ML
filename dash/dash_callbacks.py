...
def filter_available_logistic_regression_penality(value):
    ...
    return dash_utils.generate_options(solvers[value])
...
def filter_available_logistic_regression_solver(value):
    ...
    if value == 'elasticnet':
        return dash_utils.generate_options(elastic_net)
    else:
        return dash_utils.generate_options(solvers)
...
def update_hyperparameter_container(value):
    return [html.H4('Hyperparameters', style={'fontWeight': 'bold'})] + \
           dash_utils.generate_widget(value)
...
def update_output(content, name):
    if content:
        return [dash_utils.parse_contents(content, name)]
